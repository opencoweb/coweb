 ...
 * TODO Config Options
 * 
 * 1. Sync events through OT.
 * 2. Moderator Impl.
 * 3. Moderator handles late join. #1 must be true.
 * 4. 
 */
public abstract class SessionModerator {
	...
	public static synchronized SessionModerator newInstance(SessionHandler sessionHandler, 
		...
		if(SessionModerator.instance == null) {
			...
			try {
				Class<? extends SessionModerator> c = Class.forName(classStr).asSubclass(SessionModerator.class);
	...
	/**
     * Called when a client sends a sync message.  This sync event will 
     * have been processed by the operation engine.
     *
     ...
     */
    public abstract boolean onSync(ServerSession client, Message message);
    ...
    public abstract boolean canClientMakeServiceRequest(ServerSession client, Message botMessage);
