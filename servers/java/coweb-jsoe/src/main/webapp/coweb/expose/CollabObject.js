...
define([
	...
	/**
	  */
	var CollabObject = function(id) {
		this._engine = new OperationEngine(id);
		this._engine.freezeSite(id);
		this._engineFine = true;
	};

	var proto = CollabObject.prototype;

	/** TODO
	  */
	proto.isFine = function() {
		return this._engineFine;
	}
	...
	/**
	  ...
	  *
	  * @param localOp
	  * @param JSON encodable object to send to remote sites, or false on error.
	  */
	...
	proto.localEvent = function(localOp) {
		...
		if (type !== null) {
			...
			try {
				...
				console.error('CollabObject::localEvent: bad type "' + type +
	...
	/**
	  ...
	  */
	proto.remoteEvent = function(remoteOp) {
		return this._syncInbound(remoteOp.topic, remoteOp.value, remoteOp.position,
				remoteOp.site, remoteOp.sites, remoteOp.order);
	};
    ...
    proto._syncInbound = function(topic, value, type, position, site, sites, order) {
        ...
        if (sites && type) {
            ...
            try {
                ...
                console.warn("CollabObjectr: failed to push op into engine " + e.message);
				// TODO what should be done?
				this._engineFine = false;
                return false;
            }
