...
The coweb framework currently has a JavaScript client layer implemented as a set of `Dojo`_ modules using portions of `Dojo Core`_ and `CometD`_. The framework has both Java and Python server layer implementations with the former relying on CometD's Java implementation and the latter built on Facebook's `Tornado`_ server. Bayeux serves as the wire format for communication between client and server  Both server implementations support services that run within the server process and offer extension points for external services to participate in sessions.
...
.. _Dojo Core: http://dojotoolkit.org
